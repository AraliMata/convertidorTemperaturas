    ...
    func retrieveHistory(_ handler: @escaping RetrieveHistoryClosure) {
            ...
            let task = session.dataTask(with: urlRequest) {
                ...
                do {
                    ...
                    } //Traer respuesta de JSON

                    let history = try decoder.decode([TemperatureConversion].self, from: responseData) //Transformar de JSON a objeto, el objeto debe ser codable. History es un arreglo de TemperatureConversion
                    
                    if let httpResponse = response as? HTTPURLResponse {
                        print("statusCode: \(httpResponse.statusCode)")
                    }
        
                    handler(history) //El handler es el que se encarga de dar un fin util a lo que me regresó el end point
                } catch {
                    print("error trying to convert data to JSON")
                    return
                }
            ...
            task.resume() //Llama al end point
      }
